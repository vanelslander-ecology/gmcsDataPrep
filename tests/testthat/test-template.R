...
#    `functionName` is a function's name in your module (e.g., `gmcsDataPrepEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Ian/Git/gmcsDataPrep/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("gmcsDataPrep")
  path <- list(modulePath = "C:/Ian/Git",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    gmcsDataPrep = list(.saveInitialTime = NA)
  )
  ...
  if (exists("gmcsDataPrepEvent1", envir = .GlobalEnv)) {
    simOutput <- gmcsDataPrepEvent1(mySim)
  } else {
  ...
  if (exists("gmcsDataPrepEvent2", envir = .GlobalEnv)) {
    simOutput <- gmcsDataPrepEvent2(mySim)
  } else {
